...
import { ensurePetDiarySchedule, getPetDiaryForDate } from "@/lib/pet-diary-service"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = searchParams.get("ensure") === "true"
      ? await ensurePetDiarySchedule({ userId, dateKey })
      : await getPetDiaryForDate(userId, dateKey)
