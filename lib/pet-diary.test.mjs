...
test('daily diary schedules contain 4 to 8 unique sorted daytime entries', () => {
  const count = pickRandomDiaryCount(0)
  const schedule = buildRandomDiarySchedule({
    dateKey: '2026-07-14',
    count,
    randomValues: [0, 0.1, 0.2, 0.3],
  })

  assert.equal(count, 4)
  assert.equal(schedule.length, 4)
  assert.equal(new Set(schedule.map((date) => date.toISOString())).size, 4)
  assert.deepEqual(schedule, [...schedule].sort((a, b) => a.getTime() - b.getTime()))
  assert.ok(schedule.every((date) => {
    const hour = Number(date.toLocaleString('en-US', { timeZone: 'Asia/Shanghai', hour: '2-digit', hour12: false }))
