...
test("incremental seed is documented and avoids destructive operations", async () => {
  ...
  assert.match(seedSource, /id: "stu-test"/)
  assert.match(seedSource, /studentNo: "TEST001"/)
  assert.match(seedSource, /prisma\.student\.upsert/)
  assert.match(seedSource, /async function seedDemoPet/)
  assert.match(seedSource, /name: "测试心宠"/)
  assert.match(seedSource, /prisma\.pet\.create/)
