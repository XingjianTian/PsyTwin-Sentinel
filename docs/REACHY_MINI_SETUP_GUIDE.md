...
当前“实时联调”的“开始对话”使用 Gemini Live 原生语音链路。Sentinel 的 `.env.local` 需要配置 `GEMINI_API_KEY`、`GEMINI_LIVE_MODEL` 和固定的 `GEMINI_LIVE_VOICE`；语音名称先在 Google AI Studio 的 Voice Library 试听，选定后写入 `GEMINI_LIVE_VOICE` 并重启 Sentinel。会话启动后：

1. 点击“开始对话”后，桥接层不会向头部和身体下发任何姿态字段，仅左右耳朵以约 12°轻幅无声摆动；如果页面没有显示“耳朵摆动已启动”，请重启 Host Bridge 后再试。
2. 浏览器只选择实体心宠的麦克风和扬声器，未找到实体音频设备时不会回退到电脑扬声器。
3. Gemini Live 在同一个实时连接中完成语音理解、回答和原生语音输出，同时返回输入/输出转写。
4. 服务端会话提示词锁定当前学生、已保存的心宠性格配置和 OCEAN 画像；输入转写按统一风险规则累计判级。
5. “我晚上睡不好”等睡眠困扰会显示中风险；即使 Gemini 转写成“我 买 上 总 是 睡 不 好 。”这类带空格文本，也会在移除空格和标点后判级，后续普通话语不会把已识别的中风险降回低风险。
6. Gemini 开始播放回答时，实体心宠执行一次按性格配置确定的官方动作；动作请求关闭动作库 `.ogg` 音效，只保留 Gemini 语音。

页面中的“实时对话”显示 Gemini 输入/输出转写；“协作过程”显示风险识别和双层 AI 阶段摘要。连接码 `1000/1005` 表示正常结束，不应显示为初始化失败。

修改 `.env.local` 中的 `GEMINI_API_KEY`、模型或 Voice 后，必须重启 Sentinel 开发服务；只刷新浏览器不会更新已经启动的 Node 进程环境变量。
